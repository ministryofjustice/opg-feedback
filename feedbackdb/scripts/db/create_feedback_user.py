...
def create_feedback_user():
    logging.info(f"Creating {feedbackuser} user.")
...
if not inspector.has_table("perf_feedback"):
    logging.info(
        "Cannot find perf_feedback table in db. Table needs to exist in order to grant privileges to user. Exiting ...."
    )
    exit()
else:
    logging.info("Found perf_feedback table")

if feedback_user_exists():
    logging.info("feedback user already exists.")
else:
    create_feedback_user()
    if feedback_user_exists():
        logging.info("feedback user created succesfully.")
    else:
        logging.info("ERROR : creation of feedback user appears to have failed.")

if has_privileges():
    logging.info("feedback user already has privileges on perfplat table.")
else:
    grant_privileges()
    if has_privileges():
        logging.info(
            "privileges for feedback user have successfully been granted on perf_feedback table."
        )
    else:
        logging.info(
            "ERROR : granting of privileges on perf_feedback table for feedback user appears to have failed."
        )
